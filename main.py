...
def main():
    ...
    if cfg['dataset'] == 'mvtec':
        ...
        for cname in classname:
            print(f"\n======== Working on class: {cname} ========\n")
            cfg['classname'] = cname
            for shots in [1, 2, 4]:
                print(f"\n-------- Working on {shots}-shot setting --------\n")
                cfg['shots'] = shots
                print("Preparing dataset.")
                dataset = build_dataset(cfg['dataset'], cfg['root_path'], shots, cname)

                val_loader = build_data_loader(data_source=dataset.val, batch_size=64, is_train=False, tfm=preprocess, shuffle=False)
                test_loader = build_data_loader(data_source=dataset.test, batch_size=64, is_train=False, tfm=preprocess, shuffle=False)

                train_tranform = transforms.Compose([
                    transforms.RandomResizedCrop(size=336, scale=(0.5, 1), interpolation=transforms.InterpolationMode.BICUBIC),
                ...
                clip_weights = clip_classifier_for_mvtec(dataset.template, clip_model)
            ...
            print("Preparing dataset.")
            dataset = build_dataset(cfg['dataset'], cfg['root_path'], cfg['shots'], cname)
